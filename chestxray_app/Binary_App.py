...
binary_model = tf.keras.models.load_model('Final_Binary_ChestXRay.keras')
...
async def process_images(images: List[UploadFile] = File(...)):
    ...
    try:
        ...
        for image in images:
            ...
            binary_prediction = binary_model.predict(preprocessed_image)[0][0]
            binary_label = binary_labels[int(binary_prediction > 0.5)]
            #Append results to results array
            results.append([image.filename, predicted_label, f"{confidence_score:.2f}%", binary_label])
        ...
        #Write column names
        writer.writerow(["image_filename", "illness prediction", "illness confidence", "binary prediction"])
